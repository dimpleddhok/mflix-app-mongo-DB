...
def build_query_sort_project(filters):
    ...
    if filters:
        if "text" in filters:
            ...
            #  db.movies.find({genres:{$in:[‘Romance’, ‘Action’]}}, {title: 1, _id: 0, genres: 1}
...
def get_movies(filters, page, movies_per_page):
    ...
    # movies = cursor.limit(movies_per_page).skip(14)
...
def login_user(email, jwt):
    ...
    """
    try:
        jwt_token={"user_id":email,"jwt":jwt}
        # TODO: User Management
        # Use an UPSERT statement to update the "jwt" field in the document,
        # matching the "user_id" field with the email passed to this function.
        db.sessions.update_one(
            { "user_id": email },
            { "$set":jwt_token},upsert=True
        )
        return {"success": True}
    except Exception as e:
        return {"error": e}
